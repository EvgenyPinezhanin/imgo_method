...
/*     double reliability = 2.0, accuracy = 0.001;
    ...
    vector<PiyavskyGsaTask> tasks = { PiyavskyGsaTask("Sample Task №1", sampleTasks[0], 0, 1,  reliability, accuracy,
                                      ...
                                      PiyavskyGsaTask("Sample Task №2", sampleTasks[1], 0, 2,  reliability, accuracy,
                                                      0.0, maxTrials, maxFevals),
                                      PiyavskyGsaTask("Sample Task №3", sampleTasks[2], 0, 3,          2.1, accuracy,
                                                      0.0, maxTrials, maxFevals),
    ...
    for (int i = 0; i < numberTasks; i++) {
        if (tasks[i].use) {
            gsa.setProblem(tasks[i].optProblem);
            gsa.setReliability(tasks[i].reliability);
            gsa.setAccuracy(tasks[i].accuracy);
            gsa.setMaxTrials(tasks[i].maxTrials);
            gsa.setMaxFevals(tasks[i].maxFevals);
