...
const int sample_func_number = 0; // 0 - f1, 1 - f2, 2 - f3, 3 - f4
...
int main() {
    ...
    vector<task_direct> task_array = { task_direct("f1", f1, &f_data, n, vector<double>{-4.0, -4.0}, vector<double>{4.0, 4.0},
                                                   vector<double>{4.0, 4.0}, max_feval, max_iter, magic_eps, volume_reltol,
                                                   sigma_reltol, nullptr, algorithm),
                                       task_direct("f2", f2, &f_data, n, vector<double>{-4.0, -4.0}, vector<double>{4.0, 4.0},
                                                   vector<double>{1.0, 1.0}, max_feval, max_iter, magic_eps, volume_reltol,
                                                   sigma_reltol, nullptr, algorithm),
                                       task_direct("f3", f3, &f_data, n, vector<double>{-1.0, -1.0}, vector<double>{1.0, 1.0},
                                                   vector<double>{0.5, 0.5}, max_feval, max_iter, magic_eps, volume_reltol,
                                                   sigma_reltol, nullptr, algorithm),
    ...
    for (int i = 0; i < task_array.size(); i++) {
        if (task_array[i].used) {
            ...
            cout << "Number of fevals = " << data->count_evals << endl;
            cout << "X = (" << X[0] << ", " << X[1] << ")" << endl;
            cout << "f(X) = " << minf << endl;
