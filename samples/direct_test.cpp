...
int main() {
    ...
    for (int i = 0; i < taskArray.size(); i++) {
        if (taskArray[i].used) {
            ...
            addPointsGnuplot(ofstr, data->points, data->f);
        }
    }
    ofstr.close();

    size_t sizeTaskArray = taskArray.size();
    string borderNames[] = { "AX","BX", "AY", "BY" };
    for (int i = 0; i < 4; i++) {
        initArrayGnuplot(ofstrOpt, borderNames[i], sizeTaskArray);
    }
    for (int i = 0; i < sizeTaskArray; i++) {
        for (int j = 0; j < 2; j++) {
            setValueInArrayGnuplot(ofstrOpt, borderNames[2 * j], i + 1, taskArray[i].A[j], false);
            setValueInArrayGnuplot(ofstrOpt, borderNames[2 * j + 1], i + 1, taskArray[i].B[j], false);
        }
    }
    ofstrOpt.close();
