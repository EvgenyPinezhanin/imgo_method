...
const std::vector<OneDimensionalProblem> sampleTasks{
    OneDimensionalProblem([] (double x) { return -4.0 * x + 1.0; }, blockNames[0], 1,
                          opt::OneDimensionalSearchArea(3.0, 4.0), std::vector<double>{ 4.0 }, -15.0, 4.0),

    OneDimensionalProblem([] (double x) { return 5.0 * x * x + 3.0 * x - 1.0; }, blockNames[0], 2,
                          opt::OneDimensionalSearchArea(-2.0, 2.0), std::vector<double>{ -0.3 }, -1.45, 23.0),

    OneDimensionalProblem([] (double x) { return x * std::sin(x); }, blockNames[0], 3,
                          opt::OneDimensionalSearchArea(0.0, 20.0), std::vector<double>{ 17.336 }, -17.307607, 18.955),

    OneDimensionalProblem([] (double x) { return x != 0 ? x * std::sin(1 / x) : 0.0; }, blockNames[0], 4,
...
const std::vector<OneDimensionalProblem> testTasks{
    ...
    OneDimensionalProblem([] (double x) { return -(-std::sin(x) - std::sin(10.0 / 3.0 * x)); }, blockNames[1], 2,
    ...
    OneDimensionalProblem([] (double x) { return -(16.0 * x * x - 24.0 * x + 5.0) * std::exp(-x); }, blockNames[1], 4,
                          opt::OneDimensionalSearchArea(1.9, 3.9), std::vector<double>{ 2.868 }, -3.8504507, 3.0),

    OneDimensionalProblem([] (double x) { return -(-3.0 * x + 1.4) * std::sin(18.0 * x); }, blockNames[1], 5,
                          opt::OneDimensionalSearchArea(0.0, 1.2), std::vector<double>{ 0.96609 }, -1.4890725, 36.0),

    OneDimensionalProblem([] (double x) { return -((x + std::sin(x)) * std::exp(-x * x)); }, blockNames[1], 6,
    ...
    OneDimensionalProblem([] (double x) { return -(-std::sin(x) - std::sin(2.0 / 3.0 * x)); }, blockNames[1], 9,
                          opt::OneDimensionalSearchArea(3.1, 20.4), std::vector<double>{ 17.039 }, -1.9059611, 1.7),

    OneDimensionalProblem([] (double x) { return -(x * std::sin(x)); }, blockNames[1], 10,
                          opt::OneDimensionalSearchArea(0.0, 10.0), std::vector<double>{ 7.9787 }, -7.9167274, 11.0),

    OneDimensionalProblem([] (double x) { return -(-2.0 * std::cos(x) - std::cos(2.0 * x)); }, blockNames[1], 11,
                          opt::OneDimensionalSearchArea(-1.57, 6.28), std::vector<double>{ 2.094, 4.189 }, -1.4999998, 3.0),

    OneDimensionalProblem([] (double x) { return -(-std::pow(std::sin(x), 3) - std::pow(std::cos(x), 3)); }, blockNames[1], 12,
    ...
    OneDimensionalProblem([] (double x) { return -(std::exp(-x) * std::sin(2 * M_PI * x)); }, blockNames[1], 14,
                          opt::OneDimensionalSearchArea(0.0, 4.0), std::vector<double>{ 0.224885 }, -0.78868539, 6.5),

    OneDimensionalProblem([] (double x) { return -((-x * x + 5.0 * x - 6.0) / (x * x + 1)); }, blockNames[1], 15,
                          opt::OneDimensionalSearchArea(-5.0, 5.0), std::vector<double>{ 2.4142 }, -0.035533906, 6.5),

    OneDimensionalProblem([] (double x) { return -(-2.0 * (x - 3) * (x - 3) - std::exp(- x * x / 2)); }, blockNames[1], 16,
    ...
    OneDimensionalProblem([] (double x) { return -(x - std::sin(3.0 * x) + 1.0); }, blockNames[1], 19,
                          opt::OneDimensionalSearchArea(0.0, 6.5), std::vector<double>{ 5.87287 }, -7.8156745, 4.0), 

    OneDimensionalProblem([] (double x) { return -(x - std::sin(x)) * std::exp(- x * x); }, blockNames[1], 20,
