...
sampleName = "multidimensional_constrained_operational_characteristics"
...
dataFile(number) = sprintf("output_data/%s/%s/%s_%s_%s", sampleName, methodNames[1], familyName[ARG2 + 1], key[number], r[number])

if (ARG1 == 0) {
    set lmargin 10

    set title title(familyName[ARG2 + 1]) font fontName
    plot for [i = 1 : 4] dataFile(i) using 1:2 with lines lt i title "r = ".r[i].", key = ".key[i]
